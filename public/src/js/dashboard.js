document.addEventListener("DOMContentLoaded", () => {
  ...
  function showToast(message, isError = false) {
    ...
    toast.style.background = isError
      ? "linear-gradient(to right, #dc2626, #7f1d1d)" // red gradient for error
      : "linear-gradient(to right, #f43f5e, #6366f1)"; // rose-indigo for success
  ...
  addExpenseForm.addEventListener("submit", async (e) => {
    ...
    try {
      const res = await fetch("https://finora-expense-tracker-system.onrender.com/api/expenses", {
        method: "POST",
  ...
  async function fetchMonthlySummary() {
    ...
    try {
      const res = await fetch(`https://finora-expense-tracker-system.onrender.com/api/expenses/summary?month=${selectedMonth}&userId=${userId}`);
      const data = await res.json();
